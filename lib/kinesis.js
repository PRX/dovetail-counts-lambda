 ...
 */
exports.putImpressionLock = async function(redis, {listenerSession, digest, segment, ...data}) {
  const key = `dtcounts:imp:${listenerSession}/${digest}`
  ...
  if (await redis.lock(key, fld, ttl)) {
    return await exports.putImpression({...data, listenerSession, digest, segment})
  } else {
    return false
  }
}
