...
const STANDARD_EVENT = require('./standard-event.test.json')
...
describe('decoder', () => {
  ...
  it('decodes a known standard cloudfront log event', async () => {
    const results = await decoder.decodeEvent(STANDARD_EVENT)
    expect(results.length).toEqual(1)
    expect(results[0]).toEqual({
      id: `${results[0].le}/2021-04-08/${results[0].digest}`,
      le: 'Z0TmF_GE9TmFBjt05xwHbuDeoXZXyjgwbLSqMNfhM8Y',
      digest: 'AWea0j2ywM1F8_fN1dYvOq0KhhBq119BEhQ541Vd1OM',
      day: '2021-04-08',
      time: 1617840974000,
      bytes: ['3145728-6291455'],
    })
  })
