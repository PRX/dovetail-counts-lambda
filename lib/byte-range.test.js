...
describe('byte-range', () => {
  ...
  const LSD = 'byte-range/test'
  const KEY = `dtcounts:bytes:${LSD}`
  ...
  it('loads new bytes', async () => {
    const range = await ByteRange.load(LSD, redis)
    expect(range.listenerSession).toEqual('byte-range')
    expect(range.digest).toEqual('test')
  ...
  it('appends existing bytes', async () => {
    ...
    const range = await ByteRange.load(LSD, redis, '15-30,99-100')
    expect(range.listenerSession).toEqual('byte-range')
    expect(range.digest).toEqual('test')
  ...
  it('encodes byte ranges', () => {
    expect(new ByteRange(LSD).encode()).toEqual('')
    expect(new ByteRange(LSD, '10-20,21-22,5-9').encode()).toEqual('5-22')
    expect(new ByteRange(LSD, '22-23,10-20,4-12').encode()).toEqual('4-20,22-23')
  })

  it('calculates byte range intersections', () => {
    const range = new ByteRange(LSD, '4-40,100-101,120-130')
  ...
  it('checks if a segment is complete', () => {
    const range = new ByteRange(LSD, '4-40,90-100,102-130')
  ...
  it('totals bytes downloaded', () => {
    expect(new ByteRange(LSD, '').total()).toEqual(0)
    expect(new ByteRange(LSD, '0-0').total()).toEqual(1)
    expect(new ByteRange(LSD, '0-10').total()).toEqual(11)
    expect(new ByteRange(LSD, '4-40,10-20,99-100,100-101,120-130').total()).toEqual(51)
  })
