...
describe('util', () => {
  describe('#promiseAnyTruthy', () => {
    it('resolves to the first returned value', async () => {
      ...
      expect(await util.promiseAnyTruthy(p1, p2)).toEqual('p2')
    })
    ...
    it('resolves to the last falsey value', async () => {
      ...
      expect(await util.promiseAnyTruthy(p1, p2)).toEqual('')
    })
    ...
    it('picks a falsey value over an error', async () => {
      const p1 = new Promise(resolve => setTimeout(() => resolve(null), 5))
      const p2 = new Promise((_, reject) => setTimeout(() => reject('p2'), 1))
      expect(await util.promiseAnyTruthy(p1, p2)).toEqual(null)
    })
  })
})
