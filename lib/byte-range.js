...
module.exports = class ByteRange {
  ...
  constructor(listenerSessionDigest, byteRangesString) {
    const parts = listenerSessionDigest.split('/')
    if (parts.length !== 2) {
      throw new Error(`Invalid listener session digest: ${listenerSessionDigest}`)
    }
    this.listenerSession = parts[0]
    this.digest = parts[1]
    this.bytes = ByteRange.decode(byteRangesString)
  }

  static async load(lsd, redis, addBytes = null) {
    const key = `dtcounts:bytes:${lsd}`
    if (addBytes) {
      const ttl = process.env.REDIS_BYTES_TTL || DEFAULT_TTL
      const str = await redis.push(key, addBytes, ttl)
      return new ByteRange(lsd, str)
    } else {
