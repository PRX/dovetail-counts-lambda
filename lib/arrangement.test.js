...
describe('arrangement', () => {
  ...
  it('throws an error for missing arrangements', async () => {
    try {
      ...
      expect(err.skippable).toBeFalsy()
    }
  })

  it('throws an error for skippable arrangements', async () => {
    dynamo.__addArrangement(DIGEST, { skip: true })

    try {
      await Arrangement.load(DIGEST, redis)
      fail('should have gotten an error')
    } catch (err) {
      expect(err.name).toEqual('ArrangementSkippableError')
      expect(err.message).toEqual(`Skipping ${DIGEST}`)
